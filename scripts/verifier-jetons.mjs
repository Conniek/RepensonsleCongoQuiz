...
for (const fichier of await fichiersTsx(join(RACINE, "app"))) {
  ...
  const lignes = (await readFile(fichier, "utf8")).split("\n");
  lignes.forEach((ligne, index) => {
    // Toléré : un style en ligne qui ne pose QU'une propriété personnalisée
    // (« --taux »), c'est-à-dire une donnée, pas une décision d'apparence.
    const posePropriete = /"--[\w-]+"\s*:/.test(ligne) || /--[\w-]+`?\s*\]?\s*:/.test(ligne);
    if (/style=\{\{/.test(ligne) && !/width:\s*`/.test(ligne) && !posePropriete) {
